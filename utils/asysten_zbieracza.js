...
javascript: function fill(unit, number) {
  let field = $(`[name=${unit}]`);
...
if (url.indexOf('screen=place') == -1 || url.indexOf('mode=scavenge') == -1)
  ...
  var unfree_levels = doc.getElementsByClassName(
    'btn btn-default free_send_button btn-disabled',
  );
  var unlocked_levels = doc.getElementsByClassName(
    'btn btn-default free_send_button',
  );
  var free_levels = unlocked_levels.length - unfree_levels.length;

  if (free_levels == 0) alert('Brak dostÄpnych poziomĂłw zbieractwa');
  else {
    if (
      unlocked_levels.length > 1 &&
      free_levels == 1 &&
      ...
      let unit;
      for (var i = 0; i < 7; i++) {
        if (settings.archers == 0) if (i == 3 || i == 5) i++;
        if (units_settings[i].max_unit_number > 0) {
          unit = units[i];
          let field = $(`[name=${unit}]`);
          let available = Number(
            field[0].parentNode.children[1].innerText.match(/\d+/)[0],
          );
          ...
          if (unlocked_levels.length == 1) {
            ...
            let packs = 0;
            if (settings.skip_level_1 == 0) packs += 15;
            if (unlocked_levels.length >= 2) packs += 6;
            if (unlocked_levels.length >= 3) packs += 3;
            if (unlocked_levels.length == 4) packs += 2;
            ...
            if (
              (available * packs) / left_packs >
              units_settings[i].max_unit_number
            )
              to_send[i] =
                (units_settings[i].max_unit_number * packs_now) / packs;
            else to_send[i] = (available * packs_now) / left_packs;
          }
        }
      }
