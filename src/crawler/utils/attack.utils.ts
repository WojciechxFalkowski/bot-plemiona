...
export class AttackUtils {
    ...
    public static getNextTarget(villages: BarbarianVillage[], currentIndex: number): { village: BarbarianVillage; nextIndex: number } {
        ...
        this.logger.debug(`Selected target: ${targetVillage.name} (${targetVillage.coordinatesString}) at index ${targetIndex}`);
    ...
    public static async performMiniAttack(
        ...
        this.logger.log(`🗡️ Starting mini attack on ${targetVillage.name} (${targetVillage.coordinatesString})`);
        ...
        try {
            ...
            if (await attackButton.isVisible({ timeout: 5000 })) {
                ...
                if (await confirmButton.isVisible({ timeout: 5000 })) {
                    ...
                    this.logger.log(`✅ Mini attack completed successfully: ${targetVillage.name} (${targetVillage.coordinatesString})`);
            ...
            this.logger.error(`❌ Mini attack failed on ${targetVillage.name} (${targetVillage.coordinatesString}):`, error);
    ...
    public static logAttackSummary(
        ...
        if (successfulAttacks.length > 0) {
            ...
            successfulAttacks.forEach((result, index) => {
                this.logger.log(`  ${index + 1}. ${result.targetVillage.name} (${result.targetVillage.coordinatesString})`);
            });
        }
        ...
        if (failedAttacks.length > 0) {
            ...
            failedAttacks.forEach((result, index) => {
                this.logger.log(`  ${index + 1}. ${result.targetVillage.name} (${result.targetVillage.coordinatesString}) - ${result.error}`);
            });
        }
